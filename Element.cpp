	   ...
	   Global&              globdat) const
{
  ...
  pair<iter, iter> xbound = boost::minmax_element ( xCoord.begin(), 
		                             xCoord.end  () );

  xBound.first  = *xbound.first;
  xBound.second = *xbound.second;

  pair<iter, iter> ybound = boost::minmax_element ( yCoord.begin(), 
		                             yCoord.end  () );

  yBound.first  = *ybound.first;
  yBound.second = *ybound.second;

  pair<iter, iter> zbound = boost::minmax_element ( zCoord.begin(), 
		                             zCoord.end  () );
...
double Element::computeElementSizeTriangle_ ( Global&  globdat ) const
{
  ...
  double a = sqrt(pow((node2->getX() - node1->getX()),2) + pow((node2->getY() - node1->getY()),2));
  double b = sqrt(pow((node3->getX() - node2->getX()),2) + pow((node3->getY() - node2->getY()),2));
  double c = sqrt(pow((node1->getX() - node3->getX()),2) + pow((node1->getY() - node3->getY()),2));

  double s = 0.5 * (a + b + c);
  double A =  sqrt ( s * (s-a) * (s-b) * (s-c) );

  return 2.*A/(min(min(a,b),c));
